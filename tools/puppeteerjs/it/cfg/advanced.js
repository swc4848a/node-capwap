...
new Testcase({
  ...
  testcase() {
    ...
    this.wait(1000);
    this.evaluate(
      `FcldUiTest.setUiObjectValue("adminSettingsEditor-timezone", "(GMT-8:00)Pacific Time(US & Canada)")`
    );
    this.evaluate(
      `FcldUiTest.setUiObjectValue("adminSettingsEditor-ntpsync", true)`
    );
    this.evaluate(
      `FcldUiTest.setUiObjectValue("adminSettingsEditor-type", "Use FortiGuard Server")`
    );
