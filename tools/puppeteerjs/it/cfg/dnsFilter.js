...
let cloudMap = {
  ...
  "Block DNS requests to known botnet C&C":
    "label:contains(Block DNS requests to known botnet C&C)",
  ...
  "Allow DNS requests when a rating error occurs":
    "label:contains('Allow DNS requests when a rating error occurs')",
...
new Testcase({
  ...
  verify() {
    ...
    this.wait(2000);
    this.isCheck(
      gateMap["Block DNS requests to known botnet C&C"],
      apiData["blockBotnet"]
    );
    this.isCheck(
    ...
    this.isCheck(
      gateMap["Allow DNS requests when a rating error occurs"],
      apiData["errorAllow"]
    );
...
new Testcase({
  ...
  testcase() {
    ...
    this.evaluate(
      `FcldUiTest.setUiObjectValue("utmDnsFilterEditor-blockBotnet", ${!apiData[
        "blockBotnet"
      ]})`
    );
    this.evaluate(
      `FcldUiTest.setUiObjectValue("utmDnsFilterEditor-ftgdDisable", ${!apiData[
        "ftgdDisable"
      ]})`
    );
