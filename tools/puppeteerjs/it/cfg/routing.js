...
let subNetComment = "test comment edit";
...
new Testcase({
  ...
  testcase() {
    ...
    this.wait(2000);
    this.evaluate(
      `FcldUiTest.setUiObjectValue("routingEditor-destinationMode", "Subnet")`
    );
    ...
    this.evaluate(
      `FcldUiTest.setUiObjectValue("routingEditor-device", "internal")`
    );
    this.set("#fcld-routingEditor-distance", 11);
    this.evaluate(
      `FcldUiTest.setUiObjectValue("routingEditor-status", "Enabled")`
    );
...
new Testcase({
  ...
  testcase() {
    ...
    this.wait(2000);
    this.set("#fcld-routingEditor-destIp", "192.168.0.0");
    this.set("#fcld-routingEditor-destMask", "255.255.0.0");
    this.set("#fcld-routingEditor-gateway", "192.168.1.1");
    this.set("#fcld-routingEditor-distance", 12);
    this.evaluate(`FcldUiTest.setUiObjectValue("routingEditor-device", "wan")`);
    this.wait(1000);
    this.evaluate(
      `FcldUiTest.setUiObjectValue("routingEditor-status", "Disabled")`
    );
  ...
  verify() {
    ...
    this.wait(2000);
    this.isSet(gateMap["Destination IP/Netmask"], "192.168.0.0/255.255.0.0");
    this.isSet(gateMap["Gateway"], "192.168.1.1");
    this.isSet(gateMap["Administrative Distance"], 12);
    // todo: span text is empty, need support text()
    // this.isSet(gateMap['Device Interface'], "wan")
    this.isSet(gateMap["Comments"], subNetComment);
...
new Testcase({
  ...
  testcase() {
    ...
    this.wait(2000);
    this.evaluate(
      `FcldUiTest.setUiObjectValue("routingEditor-destinationMode", "Internet Service")`
    );
    this.evaluate(
      `FcldUiTest.setUiObjectValue("routingEditor-dstaddr", "none")`
    );
