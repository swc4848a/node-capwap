...
new Testcase({
  ...
  testcase() {
    ...
    this.evaluate(
      `FcldUiTest.setUiObjectValue("userSettingsEditor-authCert", "Fortinet_Factory")`
    );
  ...
  verify() {
    ...
    this.isCheck(
      gateMap["Redirect HTTP Challenge to a Secure Channel (HTTPS)"]
    );
...
new Testcase({
  name: "authentication settings modify",
  testcase() {
    this.click(cloudMap["Authentication Settings"]);
    this.wait(1000);
