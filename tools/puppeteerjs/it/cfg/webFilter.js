...
let cloudMap = {
  ...
  "FortiGuard category based filter":
    "label:contains('FortiGuard category based filter')",
  "Allow users to override blocked categories":
    "label:contains('Allow users to override blocked categories')",
  "Enforce Safe Search on Google, Yahoo!, Bing, Yandex":
    "label:contains('Enforce Safe Search on Google, Yahoo!, Bing, Yandex')",
  ...
  "Block malicious URLs discovered by FortiSandbox":
    "label:contains('Block malicious URLs discovered by FortiSandbox')",
  "Web Content Filter": "label:contains('Web Content Filter')",
  "Allow websites when a rating error occurs":
    "label:contains('Allow websites when a rating error occurs')",
  "Rate URLs by domain and IP Address":
    "label:contains('Rate URLs by domain and IP Address')",
  "Block HTTP redirects by rating":
    "label:contains('Block HTTP redirects by rating')",
  "Rate images by URL": "label:contains('Rate images by URL')",
  "Restrict Google account usage to specific domains":
    "label:contains('Restrict Google account usage to specific domains')",
  "Provide details for blocked HTTP 4xx and 5xx errors":
    "label:contains('Provide details for blocked HTTP 4xx and 5xx errors')",
  ...
  "Delete Category Usage Quota":
    "td:contains('Discrimination') ~ td div.tool_delete",
  "Choose Categories Select":
    "div.apFortiGuardCategoryActionYes:contains('Select')",
...
let gateMap = {
  ...
  "Category Usage Quota With Discrimination":
    "td.category:contains(Discrimination)",
...
new Testcase({
  ...
  verify() {
    ...
    //"proxyOptions.domainsUseGoogleAccount",
    this.isCheck(
      gateMap["FortiGuard category based filter"],
      apiData["ftgdCategoryBasedFilter.status"]
    );
    this.isCheck(
      gateMap["Allow users to override blocked categories"],
      apiData["overrideChecked"]
    );
    // this.has(gateMap['SSO_Guest_Users'])
    // this.has(gateMap['monitor-all'])
    this.isCheck(
      gateMap["Enforce Safe Search on Google, Yahoo!, Bing, Yandex"],
      apiData["searchEngineOptions.safeSearch"]
    );
    this.isCheck(
      gateMap["Log all search keywords"],
      apiData["searchEngineOptions.logSearchKeyWords"]
    );
    this.isCheck(
      gateMap["Block invalid URLs"],
      apiData["staticUrlFilter.blockInvalidURLs"]
    );
    this.isCheck(
      gateMap["Block malicious URLs discovered by FortiSandbox"],
      apiData["staticUrlFilter.urlListBySandbox"]
    );
    this.isCheck(
      gateMap["Allow websites when a rating error occurs"],
      apiData["ratingOptions.allowErrorRating"]
    );
    this.isCheck(
      gateMap["Rate URLs by domain and IP Address"],
      apiData["ratingOptions.rateByDomainAndIp"]
    );
    this.isCheck(
      gateMap["Rate images by URL"],
      apiData["ratingOptions.rateImgByURL"]
    );
    this.isCheck(
      gateMap["Provide details for blocked HTTP 4xx and 5xx errors"],
      apiData["proxyOptions.httpErrorDetail"]
    );
    this.isCheck(
      gateMap["HTTP POST Action Block"],
      apiData["proxyOptions.httpPostAction"] == "Block"
    );
    this.isCheck(
      gateMap["Remove Java Applets"],
      apiData["proxyOptions.javaFilter"]
    );
    this.isCheck(
      gateMap["Remove ActiveX"],
      apiData["proxyOptions.activeXFilter"]
    );
    this.isCheck(
      gateMap["Remove Cookies"],
      apiData["proxyOptions.cookiesFilter"]
    );
    this.isCheck(
      gateMap["Switch Duration"],
      apiData["override.durationMode"] == "Predefined"
    );
    this.isCheck(
      gateMap["URL Filter"],
      apiData["staticUrlFilter.urlFiltersEnable"]
    );
    this.isCheck(
      gateMap["Web Content Filter"],
      apiData["staticUrlFilter.contentFiltersEnable"]
    );
    this.isCheck(
      gateMap["Restrict Google account usage to specific domains"],
      apiData["proxyOptions.domainsUseGoogleAccountEnable"]
    );
    ...
    this.isSet(
      gateMap["Duration Minutes"],
      apiData["override.duration-minute"]
    );
  }
});
...
new Testcase({
  ...
  testcase() {
    ...
    this.wait(1000);
    this.evaluate(
      `FcldUiTest.setUiObjectValue("utmWebFilterEditor-override.durationMode", "Ask")`
    );
    this.evaluate(
      `FcldUiTest.setUiObjectValue("utmWebFilterEditor-override.scope", "Ask")`
    );
    this.click(cloudMap["Save"]);
  },
  verify() {
...
new Testcase({
  ...
  testcase() {
    ...
    this.wait(1000);
    this.evaluate(
      `FcldUiTest.setUiObjectValue("utmWebFilterEditor-ftgdCategoryBasedFilter.status", ${!apiData[
        "ftgdCategoryBasedFilter.status"
      ]})`
    );
    this.evaluate(
      `FcldUiTest.setUiObjectValue("utmWebFilterEditor-overrideChecked", ${
        apiData["overrideChecked"]
      })`
    );
    this.click(cloudMap["Save"]);
