...
let cloudMap = {
  ...
  "Delete Virtual IP One":
    "td.left:contains('" + name + "')~td.right div[title='Delete']",
  "Delete Virtual IP Group":
    "td.left:contains('" + grpName + "')~td.right div[title='Delete']",
...
new Testcase({
  ...
  testcase() {
    ...
    this.evaluate(`FcldUiTest.setUiObjectValue("vipEditor-extintf", "any")`);
    this.wait(1000);
    this.evaluate(
      `FcldUiTest.setUiObjectValue("vipEditor-extip0", "${extip0}")`
    );
    this.evaluate(
      `FcldUiTest.setUiObjectValue("vipEditor-extip1", "${extip1}")`
    );
    this.evaluate(
      `FcldUiTest.setUiObjectValue("vipEditor-mappedip0", "${mappedip0}")`
    );
    ...
    this.evaluate(
      `FcldUiTest.setUiObjectValue("vipEditor-protocol", "${protocol}")`
    );
    this.evaluate(
      `FcldUiTest.setUiObjectValue("vipEditor-extport0", "${extport0}")`
    );
    this.evaluate(
      `FcldUiTest.setUiObjectValue("vipEditor-extport1", "${extport1}")`
    );
    this.evaluate(
      `FcldUiTest.setUiObjectValue("vipEditor-mappedport0", "${mappedport0}")`
    );
...
new Testcase({
  ...
  testcase() {
    ...
    this.evaluate(
      `FcldUiTest.setUiObjectValue("vipgrpEditor-interface", "${extintf}")`
    );
    this.evaluate(
      `FcldUiTest.setUiObjectValue("vipgrpEditor-member", ["${name}"])`
    );
