...
local pf_preamble = ProtoField.new   ("Preamble", "ftnt.capwap.preamble", ftypes.NONE)
...
local pf_header = ProtoField.new   ("Header", "ftnt.capwap.header", ftypes.NONE)
...
local pf_control_header = ProtoField.new("Control Header", "ftnt.capwap.control.header", ftypes.NONE)
...
local pf_message_element = ProtoField.new("Message Element", "ftnt.capwap.message.element", ftypes.NONE)

capwap.fields = {
    pf_preamble, pf_preamble_version, pf_preamble_type,
    pf_header, pf_header_length, pf_header_radio_id, pf_header_binding_id, pf_header_flags, pf_header_fragment_id, pf_header_fragment_offset, pf_header_reserved,
    pf_control_header, pf_control_header_message_type, pf_control_header_message_type_enterprise_number, pf_control_header_message_type_enterprise_specific, 
...
function capwap.dissector(tvbuf,pktinfo,root)
    ...
    local header_tree = tree:add(pf_header,  tvbuf:range(1,7))
    ...
    local control_header = tree:add(pf_control_header, tvbuf:range(8,8))
    ...
    local message_element = tree:add(pf_message_element, tvbuf:range(16))
